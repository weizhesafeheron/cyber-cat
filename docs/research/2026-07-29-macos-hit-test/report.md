...
**这个工具有一条重要边界（2026-07-29 补记，由 ticket #5 的实现过程发现）：`NSWindow.windowNumber(at:belowWindowWithWindowNumber:)` 只能看到调用进程自己的窗口。**
本报告的全部探测都在同一个进程内自建全部窗口，因此结论有效。
但**不能跨进程复用** - 拿它从外部程序去观测另一个应用（比如真实的宠物窗口）的事件路由，任何位置都只会返回 0，而同进程自建的对照窗口却能被正确命中。后续不要再往这条路上花时间。
...
## 六、后续实测补充（2026-07-29，ticket #5 验收）

在真实应用上用真实鼠标验收了三种点击（下层放一个记录点击次数的 AppKit 窗口）：点猫身体 → 猫有反应且下层计数不变；点透明区 → 下层计数递增且猫无反应；重复点猫 → 表现一致。

**`acceptsFirstMouse` 的风险没有发生。** 实现阶段曾担心 macOS 上点击非活跃窗口的第一下只用于激活、不下发给视图（`acceptsFirstMouse` 默认 false）。实测第一下就生效，因此不需要用 objc2 覆写它。这条风险可以划掉。

`baseMargin = 3` 精灵像素（约 9 个逻辑点）的提前量在实际使用中没有产生「猫周围黏手」的感觉。
