...
describe('气泡在猫头顶，且不压到猫', () => {
  ...
  it('任何品种、任何自主动作、任何一帧，猫本体都不会伸进气泡的行里', () => {
    // 这是 BUBBLE_BASE_SPRITE_Y = 5 的依据。目测「气泡在头顶上方」在趴睡与扑跳
    // 之间差了十几行，只有把全部姿态跑一遍才知道最靠上的那一帧在哪。
    //
    // **运动层专属的动作不在这里比。** 被拎起来会把猫整只抬高十六个精灵像素、
    // 必然伸进气泡那几行 - 那种情况下气泡本来就不画（bubbleVisible 的第三个参数），
    // 由下面单独一条测试守着。
    ...
    for (const breed of breeds) {
      for (const seed of [1, 7, 42, 1234, 20260728]) {
        ...
        for (const key of ACTION_KEYS) {
          if ((MOTION_ONLY_ACTIONS as readonly ActionKey[]).includes(key)) continue;
...
describe('猫被拎在手里时不画气泡', () => {
  it('拎着或下落中一律不画，但气泡没有被点掉 - 放下之后还在', () => {
    const armed = 1000;
    const during = armed + 5_000;
    // 在地面上：该画
    expect(bubbleVisible(armed, during, false)).toBe(true);
    // 离地：不画
    expect(bubbleVisible(armed, during, true)).toBe(false);
    // 放回地面：又画出来了（说明只是不画，不是点掉）
    expect(bubbleVisible(armed, during + 1, false)).toBe(true);
  });

  it('寿命到了就算在地面上也不画', () => {
    const armed = 0;
    expect(bubbleVisible(armed, BUBBLE_LIFE_MS + 1, false)).toBe(false);
  });

  it('从来没冒过的时候，离不离地都不画', () => {
    expect(bubbleVisible(null, 5_000, false)).toBe(false);
    expect(bubbleVisible(null, 5_000, true)).toBe(false);
  });

  it('被拎起来的姿态确实会伸进气泡那几行 - 否则上面那条约束是空的', () => {
    const renderer = new CatRenderer();
    const cat = makeCat('orange', 20260728);
    const mi = { eyeOpen: 1, earFlickL: 0, earFlickR: 0, tilt: 0 };
    const bottom = bubbleSpriteRect(0).y1;
    let topmost = H;
    for (const t of [0, 0.4, 1.1, 2.0]) {
      const res = renderer.render(cat, ACTIONS.held.make(t, cat, mi));
      for (let y = 0; y < topmost; y++) {
        for (let x = 0; x < W; x++) {
          if (res.pixels[(y * W + x) * 4 + 3] !== 0) {
            topmost = y;
    ...
    expect(topmost, `被拎起来时最高的一行是 ${topmost}，气泡底边是 ${bottom}`).toBeLessThan(bottom);
  });
});
