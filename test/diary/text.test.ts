...
describe('认不出的事件不能把整份日记搞挂', () => {
  it('给一句兜底，不抛错也不露出英文名', () => {
    const cat = makeCat('orange', 20260728);
    // 存档可以比代码新（用户降级了版本），档案里还留着历任猫的日记。
    // 抛出去的话用户丢的是整个日记窗口，而不是一条。
    const text = diaryText(
      { kind: 'someFutureKind' as WorldEventKind, at: 0, important: false },
      cat,
    );
  ...
  it('对照组：认得出的事件不会掉进兜底', () => {
    const cat = makeCat('orange', 20260728);
    const known = diaryText({ kind: 'ate', at: 0, important: false }, cat);
    const unknown = diaryText(
      { kind: 'nope' as WorldEventKind, at: 0, important: false },
      cat,
    );
