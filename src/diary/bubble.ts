 ...
 * 这一帧该不该画气泡。
 *
 * 除了寿命，还有一条：**猫被拎在手里（或正在下落）时不画。**
 * 气泡占的是精灵缓冲上方那几行，而被拎起来的姿态会把猫整只抬高十六个精灵像素，
 * 正好伸进那几行 - 头顶挂着思考气泡的同时被人拎在半空，读起来是两件事叠在一起。
 * 而且那时用户的手正在猫身上，气泡的点击区还会跟拖拽抢同一片像素。
 *
 * 只是不画，不是点掉：放下之后气泡还在，寿命也照旧走。
 */
export function bubbleVisible(armedAtMs: number | null, nowMs: number, offGround: boolean): boolean {
  return !offGround && bubbleAlive(armedAtMs, nowMs);
}
