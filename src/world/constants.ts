...
/**
 * 饱食度低于此值时，饿会压过作息节律：猫睡不安稳，睡着了也会被饿醒。
 *
 * **为什么必须有这条。** 吃饭要求猫是醒的（真猫不会睡着吃），而深夜的入睡倾向是
 * 0.85、醒来概率只有每半小时 4.5% - 于是一只饿着的猫可以睡满整夜，**碗里的粮
 * 一口不动**，饱食度一路掉到 0 开始挨饿。真机上撞到过：饱食度 26.9、碗里三份粮，
 * 连跑三个模拟步全在睡，饱食度只降不升。
 * 用户看到的是「碗是满的，猫却饿病了」，那读起来是系统坏了，不是猫有个性。
 *
 * 取 30 而不是 HUNGRY_VISIBLE_THRESHOLD 的 25：**要比「饿到有可视表现」更早生效**，
 * 这样猫是醒着走到食盆边的，用户才看得见那个信号（issue #13 的验收项
 * 「挨饿有明显的可视表现，玩家有机会发现并挽回」）。
 *
 * 这条**不推翻**「睡着的可能睡完这觉」（mvp-scope 第 4 节的邀请式语义）：
 * 阈值之上一切照旧，添粮之后不饿的猫该睡完就睡完。
 */
export const HUNGER_WAKES_THRESHOLD = 30;

/**
 * 饿着时入睡意愿的折减倍率。
 *
 * 不设成 0：饿也不该让猫永远不睡 - 累垮那条（ENERGY_EXHAUSTED）仍然优先，
 * 而且「饿得睡不安稳」比「饿了就绝不睡」更像真猫。
 */
export const HUNGER_SLEEP_DAMP = 0.15;

/**
 * 饿着时每个模拟步被饿醒的概率。
 *
 * 取到「一两步内必醒」的量级：0.6 时两步内醒来的概率是 84%，也就是一小时以内。
 * 再低就救不回来了 - 从阈值 30 掉到 0 只有五个小时。
 *
 * **要求精力高于累垮线才会被饿醒。** 不加这条会在「饿醒 → 精力耗尽累倒 → 又被饿醒」
 * 之间来回抖，猫一直在闪。
 */
export const HUNGER_WAKE_CHANCE = 0.6;
